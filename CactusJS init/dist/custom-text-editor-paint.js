...
['play', 'copy'].forEach(function (img) { return loadImg(img); });
var theme = null;
...
function render() {
    ...
    var tabWidth = 140;
    editor.tabs.forEach(function (tab, i) {
        var x = i * (tabWidth + 1);
        ctx.fillStyle = theme.editor_bg;
...
function getCharWidth(c, x) {
    if (x === void 0) { x = 8; }
    return Math.floor(ctx.measureText(c).width + 1);
    if ("mw".indexOf(c.toLowerCase()) >= 0)
        x *= 1.5;
    else if (".*/?r\"[]{}".indexOf(c) >= 0)
        x *= 0.75;
    else if ("tlifI|!():".indexOf(c) >= 0)
        x *= 0.5;
    else if ("RCD+".indexOf(c) >= 0)
        x *= 1.25;
    return x;
}
...
//# sourceMappingURL=custom-text-editor-paint.js.map