var commonFileExtensions = ['md', 'css', 'cs', 'txt', 'js', 'ts', 'html', 'json', 'compute', 'py'];
var pyKeywords = ["max", "min", "len", "isinstance", "print", "float", "str", "int", "and", "as", "assert", "async", "open", "save", "await", "break", "class", "continue", "def", "del", "elif", "else", "except", "finally", "for", "from", "if", "import", "in", "is", "not", "or", "pass", "raise", "return", "try", "while", "with", "yield"];
var pyValues = ["True", "False", "None", "lambda", "global", "nonlocal"];
var csKeywords = ["public", "private", "protected", "internal", "if", "else", "for", "while", "do", "switch", "case", "default", "break", "continue", "try", "catch", "new"];
var csTypes = ["bool", "byte", "char", "decimal", "double", "float", "int", "long", "string", "uint", "ulong", "ushort", "void", "object",];
var allCsKeywords = ["add",
    "and",
    "alias",
    "ascending",
    "args",
...
function insert(text, i, delCount, str) {
    return text.substring(0, i) + str + text.substring(i + delCount);
}
function parseCombined(text, lang) {
    ...
    if (lang == 'md') {
        ...
        return parseCode(text, lang);
}
var _q = "\u0091"; // signifies text formatting, is ignored in rendering
var sectionCodeStart = "\u0091C1";
var sectionCodeEnd = "\u0091C2";
function printFormat(italic, bold, underline, strikeThrough) {
    if (italic === void 0) { italic = false; }
    if (bold === void 0) { bold = false; }
    if (underline === void 0) { underline = false; }
    if (strikeThrough === void 0) { strikeThrough = false; }
    return _q + "f" + (italic ? "i" : ".") + (bold ? "b" : ".") + (underline ? "u" : ".") + (strikeThrough ? "s" : ".");
}
/// use 6 digit hex colors!
function printColor(color) {
    if (color === void 0) { color = "d2d2d2"; }
    return _q + "c" + color;
}
function hyperlink(text, url) {
    return _q + "a" + url + _q + "a" + text + _q + "a";
}
function move_y(y) { return _q + "y" + y + _q; }
function move_x(x) { return _q + "x" + x + _q; }
function draw_line(dx, dy, w, h) { return _q + "l" + dx + "," + dy + "," + w + "," + h + _q; }
var verticalPadding = _q + "\n";
var horizontalPadding = _q + "\t";
var normalSize = _q + "1";
var header2 = _q + "2";
var header1 = _q + "3";
var noformat = printFormat();
var nocol = printColor();
var fontMath = _q + "Fmath";
var fontNormal = _q + "Fnorm";
var c_comment = printColor("6272a4");
var c_warning = printColor("db441e");
var c_question = printColor("20aabd");
var c_highlighted = printColor("5882ff");
var c_number = printColor("50e678");
var c_self = printColor("bd93fa");
var c_keyword = printColor("ff78be");
var c_function = printColor("ffed4b");
var c_property = printColor("8cdcfa");
var c_operator = printColor("c8a04b");
var c_string = printColor("e29272");
function parseMd(text) {
    ...
    // md language
    function wrap(findIn, findOut, replaceIn, replaceOut) {
        if (findIn === void 0) { findIn = '# '; }
        if (findOut === void 0) { findOut = '\n'; }
        if (replaceIn === void 0) { replaceIn = '<h1>'; }
        if (replaceOut === void 0) { replaceOut = '</h1>'; }
        var i = 0;
        while (i >= 0) {
    ...
    while (i >= 0) {
        ...
        var url = s[0];
        var surl = url.split("/"); //this avoids pesky ../ urls being mistaken for file extensions
        surl = surl[surl.length - 1];
        var extension = surl.split(".");
        if (extension.length > 0)
            extension = extension[extension.length - 1];
        else
            extension = null;
        if (!(url.startsWith("http://") || url.startsWith("https://")) && !commonFileExtensions.includes(extension))
            url = "http://" + url;
    ...
    for (var i = 0; i < text.length - 1; i++) {
        ...
        if (c == '\\') {
            ...
            if (cmd.startsWith('frac') || cmd.startsWith('tfrac')) {
                ...
                if (pw > qw)
                    var insertion = move_x(3) + move_y(-7) + p + move_y(14) + move_x(dw - pw);
                else
                    var insertion = move_x(3 + dw) + move_y(-7) + p + move_y(14) + move_x(-pw - dw);
                ...
                text = insert(text, i, 6, "α");
            else if (cmd.startsWith("beta"))
                text = insert(text, i, 5, "β");
            else if (cmd.startsWith("gamma"))
                text = insert(text, i, 6, "γ");
            else if (cmd.startsWith("delta"))
                text = insert(text, i, 6, "δ");
            else if (cmd.startsWith("epsilon"))
                text = insert(text, i, 8, "ε");
            else if (cmd.startsWith("intersect"))
                text = insert(text, i, 10, "∩");
            else if (cmd.startsWith("int"))
                text = insert(text, i, 4, "∫");
            else if (cmd.startsWith("in"))
                text = insert(text, i, 3, "∈");
            else if (cmd.startsWith("cdot"))
                text = insert(text, i, 5, "·");
            else if (cmd.startsWith("sum"))
                text = insert(text, i, 4, "∑ ");
            else if (cmd.startsWith("prod"))
                text = insert(text, i, 5, "∏ ");
            else if (cmd.startsWith("sqrt"))
                text = insert(text, i, 5, "√");
            else if (cmd.startsWith("nabla"))
                text = insert(text, i, 6, "∇");
            else if (cmd.startsWith("union"))
                text = insert(text, i, 6, "∪");
            else if (cmd.startsWith("notin"))
                text = insert(text, i, 3, "∉");
            else if (cmd.startsWith("forall"))
                text = insert(text, i, 7, "∀");
            else if (cmd.startsWith("partial"))
                text = insert(text, i, 8, "∂");
            else if (cmd.startsWith("uparrow"))
                text = insert(text, i, 8, "↑");
            else if (cmd.startsWith("downarrow"))
                text = insert(text, i, 10, "↓");
            else if (cmd.startsWith("leftarrow"))
                text = insert(text, i, 10, "←");
            else if (cmd.startsWith("rightarrow"))
                text = insert(text, i, 11, "→");
        }
        prevC = c;
    }
    return text;
...
function parseCode(text, lang) {
    ...
    var stringChars = ["'", '"', '`'];
    var char_breakOutOfString = null;
...
function getLang(text) {
    if (text === void 0) { text = ""; }
    if (text.includes("```"))
        return "md"; //this would otherwise throw us off 
    ...
    for (var i = 0; i < commonFileExtensions.length; i++)
        if (firstLine.startsWith(commonFileExtensions[i]))
            return commonFileExtensions[i];
    ...
    if (start.includes('using') || start.includes('public'))
        return 'cs';
    if (start.includes('"""'))
        return 'py';
    ...
    if (text[0] != text[0].toUpperCase())
        return "py"; // short lines but few semicolons usually means py
    return 'md'; //we dont really know, fallback to md
}
// used to discriminate between py and md
function words(text) {
    var words = text.split(/\s+/).filter(function (word) { return /^[A-Za-z]+$/.test(word); });
    return words.length;
}
// used in code formatting
function attributeEndIdx(str, i) {
    var regex = /[^a-zA-Z0-9_]/;
    var match = str.slice(i).match(regex);
    if (match) {
