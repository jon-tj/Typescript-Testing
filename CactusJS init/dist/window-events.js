var keys = {};
var scrollTopOffset = 50; // distance from top the scrollCapture sits on
//#region Canvas only events
canvas.addEventListener("mouseenter", function (e) { render(); });
scrollCapture.addEventListener("scroll", function (e) {
    ...
    if (tab != null) {
        if (keys['Shift']) {
            ...
            */
            ...
            if (tab.scrollY < 0)
                tab.scrollY = 0;
            if (tab.scrollY >= tab.pageHeight)
                tab.scrollY = tab.pageHeight;
        }
        render();
    }
});
...
window.addEventListener("resize", resizeCanvas = function () {
